...
type interpreter_error =
  | StackUnderflow
  | TypeError of value
  | Break of interpreter (* abusively using an error to implement breaks in loops *)
  | InvalidDereference
...
let builtin_of_string string =
  ...
  | "." -> Some (Builtin (fun i -> match pop i with
    | Ok (v, i) -> (match v with
        | Int n -> i.put_string ((string_of_int n) ^ " ")
        | Float f -> i.put_string ((string_of_float f) ^ " ")); 
      Ok i
    | Error e -> Error e))
  (* print a new line (--) *)
  | "cr" -> Some (Builtin (fun i -> i.put_string "\n"; Ok i))
  (* print the top of the stack as if it was an ascii characer (v --) *)
  | "emit" -> Some (Builtin (fun i -> match pop i with
    | Ok (v, i) -> (match v with
      | Int n -> i.put_string (String.make 1 (char_of_int n)); Ok i
      | Float _ -> Error (TypeError v))
    | Error e -> Error e ))
  (* read in a character from stdin (-- c) *)
  | "key" -> Some (Builtin (fun i -> Ok (push i (Int (int_of_char (i.get_char ()))))))
  ...
  if (cc != 0) then (* handle comments *)
    ...
    | String ss :: state -> if (String.ends_with ~suffix:"\"" word)
      then let s = string_of_string_list (List.rev ((String.sub word 0 ((String.length word) - 1)) :: ss)) in
        append_word ~s:state (Builtin (fun i -> i.put_string s; Ok i))
